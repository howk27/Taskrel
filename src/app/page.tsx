...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-[var(--tr-bg)] text-white">
      <nav className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 md:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/signup"
            className="hidden rounded-lg bg-[var(--tr-orange)] px-4 py-2 text-sm font-black text-[#241205] transition-colors hover:bg-[var(--tr-amber)] sm:inline-flex"
          >
            {publicLaunch.navCta}
          </Link>
        </div>
      ...
      <section className="mx-auto grid min-h-[calc(100vh-88px)] w-full max-w-7xl items-start gap-12 px-5 pb-12 pt-7 md:grid-cols-[0.92fr_1.08fr] md:px-8 md:pb-16 md:pt-14 lg:gap-16">
        <div className="max-w-2xl">
          <div className="inline-flex items-center gap-2 rounded-lg border border-[var(--tr-orange)]/30 bg-[var(--tr-orange)]/10 px-3 py-2 text-xs font-bold text-[var(--tr-amber)]">
            {publicLaunch.eyebrow}
          </div>

          <h1 className="mt-7 max-w-2xl text-4xl font-extrabold leading-[1.05] text-white sm:text-5xl lg:text-6xl">
          ...
          <div className="mt-8 flex flex-col gap-3 lg:flex-row">
            <Link
              href="/signup"
              className="inline-flex h-13 items-center justify-center rounded-lg bg-[var(--tr-orange)] px-6 text-base font-bold text-[#241205] transition-colors hover:bg-[var(--tr-amber)]"
            >
        ...
        <div className="grid gap-3 sm:grid-cols-3 md:col-span-2">
          {trades.map(({ label, Icon }) => (
            <div key={label} className="flex items-center gap-3 rounded-lg border border-[var(--tr-border-soft)] bg-[var(--tr-surface)] px-4 py-3">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-[var(--tr-info)]/12 text-[var(--tr-info)]">
      ...
      <section className="border-y border-[var(--tr-border-soft)] bg-[var(--tr-bg-soft)] px-5 py-10 md:px-8">
        <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-4">
          {workflow.map(({ label, Icon }, index) => (
            <div key={label} className="flex items-center gap-4 rounded-lg border border-[var(--tr-border-soft)] bg-[var(--tr-surface)] p-4">
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-5 py-16 md:grid-cols-[0.9fr_1.1fr] md:px-8 md:py-24">
        <div>
          <p className="text-sm font-bold text-[var(--tr-amber)]">Built around the daily quote cycle</p>
          ...
          <p className="mt-5 max-w-xl text-base leading-7 text-[var(--tr-text-muted)]">
            Taskrel focuses on the work contractors repeat: quote, review, schedule, invoice. Pricing can be edited before anything goes to a client, and Taskrel remembers those rates for future quotes.
          </p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <InfoPanel title="Launch-ready records" body="Email delivery, Stripe payments, and exports keep the quote-to-cash trail connected." tone="violet" />
        </div>
      </section>

      <section className="px-5 pb-16 md:px-8 md:pb-24">
        <div className="mx-auto grid max-w-7xl gap-8 rounded-xl border border-[var(--tr-border-soft)] bg-[var(--tr-surface)] p-6 md:grid-cols-[1fr_360px] md:p-8">
          <div>
            <TaskrelWordmark size="md" />
            <h2 className="mt-6 text-3xl font-extrabold leading-tight text-white">{publicLaunch.proofHeading}</h2>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/signup"
                className="inline-flex h-12 items-center justify-center rounded-lg bg-[var(--tr-orange)] px-5 text-sm font-bold text-[#241205] hover:bg-[var(--tr-amber)]"
              >
                {publicLaunch.primaryCta}
              </Link>
              <Link
          ...
          <div className="rounded-lg border border-[var(--tr-border-soft)] bg-[var(--tr-bg-soft)] p-5">
            <p className="text-sm font-black uppercase text-[var(--tr-text-faint)]">{publicLaunch.pricingLabel}</p>
      ...
      <footer className="border-t border-white/10 px-5 py-8 text-xs text-[var(--tr-text-faint)]">
...
function HeroVisual() {
  return (
    <div className="relative">
      <div className="rounded-xl border border-[var(--tr-border-soft)] bg-[var(--tr-shell)] p-3">
        <div className="grid gap-3 lg:grid-cols-[260px_1fr]">
          <div className="rounded-lg border border-[var(--tr-border-soft)] bg-[var(--tr-bg-soft)] p-4">
            <div className="mb-5 flex items-center justify-between">
              <TaskrelWordmark size="sm" />
              <span className="rounded-full bg-[var(--tr-green)]/12 px-2.5 py-1 text-[10px] font-bold uppercase text-[var(--tr-green)]">Live</span>
            </div>
          ...
          <div className="rounded-lg border border-[var(--tr-border-soft)] bg-[var(--tr-surface)] p-4">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold text-[var(--tr-text-faint)]">Quote preview</p>
                <p className="mt-1 text-lg font-extrabold text-white">Kitchen cabinet repaint</p>
              </div>
              <span className="rounded-lg bg-[var(--tr-orange)] px-3 py-2 text-xs font-bold text-[#241205]">Review & Send</span>
            </div>

            <div className="grid gap-4 xl:grid-cols-[1fr_220px]">
              <div className="rounded-lg bg-[#fffaf5] p-4 text-[#1f2937]">
                <div className="flex items-start justify-between gap-4 border-b border-[#fed7aa] pb-4">
                  <div>
                    <div className="mb-3 grid h-12 w-20 place-items-center rounded-lg border border-dashed border-[#b45309] text-[10px] font-bold uppercase text-[#b45309]">Logo</div>
                    <p className="font-extrabold">Deivi Painting Co.</p>
                    <p className="text-xs text-[#9a3412]">Licensed & insured</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-bold uppercase text-[#9a3412]">Total</p>
                    <p className="text-2xl font-extrabold">$4,820</p>
                  </div>
                </div>
                ...
                <div className="mt-4 rounded-lg border border-[#fed7aa] bg-[#fff7ed] p-3">
                  <p className="text-[10px] font-bold uppercase text-[#b45309]">Policies & warranty</p>
...
function QueueCard({
  ...
  const toneClass = {
    blue: "text-[var(--tr-info)] bg-[var(--tr-info)]/10",
    amber: "text-[var(--tr-amber)] bg-[var(--tr-amber)]/10",
    green: "text-[var(--tr-green)] bg-[var(--tr-green)]/10",
  }[tone];

  return (
    <div className="rounded-lg border border-[var(--tr-border-soft)] bg-[var(--tr-surface)] p-3">
      <div className="flex items-center justify-between gap-3">
        <p className="truncate text-sm font-bold text-white">{client}</p>
        <span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase ${toneClass}`}>{status}</span>
      </div>
