      ...
      <div className="mx-auto w-full max-w-md space-y-6">
        <Surface className="p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--tr-text-muted)]">
            Tell Taskrel what work you quote so estimates, scheduling, and invoices start with the right defaults.
          </p>
        </Surface>
        ...
        <form action={action} className="space-y-6">
          ...
          <section>
            <h2 className="mb-3 text-sm font-semibold text-white">What kind of contractor business do you run?</h2>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 rounded-lg border p-3 text-left transition-colors ${
                      active
                        ? "border-[var(--tr-blue)] bg-[var(--tr-blue)]/12 text-white"
                        : "border-[var(--tr-border)] bg-[var(--tr-surface)] text-[var(--tr-text-muted)] hover:border-slate-600"
                    }`}
                  >
                    <IconComponent size={22} weight={active ? "fill" : "regular"} className={active ? "text-[var(--tr-blue)]" : "text-slate-400"} />
          ...
          <section>
            <h2 className="mb-3 text-sm font-semibold text-white">Which trades do you quote?</h2>
                ...
                return (
                  <button
                    ...
                    className={`flex min-h-24 flex-col justify-between rounded-lg border p-3 text-left transition-colors ${
                      active
                        ? "border-[var(--tr-blue)] bg-[var(--tr-blue)]/12 text-white"
                        : "border-[var(--tr-border)] bg-[var(--tr-surface)] text-slate-300 hover:border-slate-600"
                    }`}
                  >
                    <IconComponent size={24} weight={active ? "fill" : "regular"} className={active ? "text-[var(--tr-blue)]" : "text-slate-400"} />
          ...
          {primaryOptions.length > 0 && (
            <section>
              ...
              <select
                ...
                className="tr-input h-12 w-full rounded-lg px-3 text-sm font-medium text-white"
              >
          ...
          {state?.error && <p className="rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">{state.error}</p>}
